...
func checkPos(t *testing.T, msg string, got, want token.Position) {
...
func TestFileSetPastEnd(t *testing.T) {
	...
	if f := fset.File(token.Pos(fset.Base())); f != nil {
...
`)
	...
	for i, offs := range f.lines {
		...
		want := token.Position{filename, offs, i + 1, 1}
